...
const userDao = {
    ...
    joinRoom: async (userId, roomId) => {
        try {
            const query = 'INSERT INTO enrolled_players (users_id, rooms_id) VALUES (?, ?)';
    ...
    getEnrolledPlayersCount: async (roomId) => {
        try {
            const query = 'SELECT COUNT(*) AS count FROM enrolled_players WHERE rooms_id = ?';
    ...
    createRoom: async() => {
        try {
            const query = 'INSERT INTO rooms (max_players, time, round, is_private, created_at) VALUES (DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT);'
            return await appDataSource.query(query);
        }catch{
